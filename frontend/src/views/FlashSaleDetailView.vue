...
import { Loader2, ShieldCheck, Zap, Server, Activity, ArrowLeft, Bot, Terminal, TrendingUp, Clock } from 'lucide-vue-next'
  ...
  <div v-else class="min-h-screen pb-32 lg:pb-20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-8">
      ...
      <div class="lg:col-span-7 space-y-8">
        <div class="aspect-square lg:aspect-[16/10] bg-surface-light border border-white/5 relative overflow-hidden group">
          <SmartImage
          ...
          <div class="grid grid-cols-3 gap-8 mb-6">
            <div>
              ...
              <div class="flex items-end gap-2">
                <div class="text-3xl font-mono font-bold text-white">{{ aiAnalysis.difficulty }}</div>
                <div class="text-sm text-tertiary mb-1">/ 10</div>
              </div>
              <div class="h-1 w-full bg-surface-light mt-2 rounded-full overflow-hidden">
                <div class="h-full bg-accent" :style="{ width: `${aiAnalysis.difficulty * 10}%` }"></div>
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <div class="text-xl font-mono font-bold text-white uppercase">{{ aiAnalysis.risk }}</div>
            </div>
          </div>
      ...
      <div class="lg:col-span-5 relative">
        <div class="lg:sticky lg:top-32 space-y-8">
          ...
          <div class="p-6 bg-surface-light/50 border border-white/5 backdrop-blur-sm">
            ...
            <div class="flex items-center gap-2 text-xs text-accent uppercase tracking-widest font-bold">
          ...
          <div class="space-y-4">
            <div class="flex justify-between items-end border-b border-white/10 pb-2">
              ...
              <span class="font-mono text-xl text-white">{{ currentStock }} <span class="text-tertiary text-sm">/ {{ detail.flash_sale.total_stock }}</span></span>
            </div>
          ...
          <div class="hidden lg:block pt-4">
            ...
            <button 
              ...
              :class="[
                ...
                rushStatus === 'success' ? 'bg-green-600 text-white' :
                currentStock <= 0 ? 'bg-surface-light text-tertiary' :
                'bg-accent text-white hover:bg-accent-hover'
              ]"
            >
              <template v-if="rushLoading"><Loader2 class="w-6 h-6 animate-spin" /> PROCESSING</template>
    ...
    <div class="lg:hidden fixed bottom-0 left-0 w-full z-50 bg-background/90 backdrop-blur-xl border-t border-white/10 p-4 pb-8">
      <div class="flex gap-4 items-center">
        <div class="flex-1">
          <div class="text-xs text-secondary uppercase tracking-wider mb-1">Flash Price</div>
          <div class="text-2xl font-mono font-bold text-white">¥{{ detail.flash_sale.flash_price }}</div>
        </div>
        <button 
