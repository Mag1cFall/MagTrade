<template>
  <div class="py-20 bg-black relative overflow-hidden border-b border-white/5 platform-stats">
    <div
      class="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-accent/5 via-transparent to-transparent"
    ></div>

    <div class="max-w-6xl mx-auto px-4 relative z-10">
      <div class="text-center mb-16">
        <h2
          class="text-3xl md:text-5xl font-black text-white mb-4 uppercase tracking-tighter italic"
        >
          Platform Statistics
        </h2>
        <p class="text-secondary font-mono tracking-widest text-xs uppercase opacity-60">
          Real-time throughput metrics // Network Load
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
        ...
        >
          <div class="flex flex-col items-center justify-center space-y-2">
            <div
              class="text-3xl sm:text-4xl lg:text-5xl font-black text-white font-mono group-hover:text-accent transition-colors leading-none tracking-tighter"
            >
              <span>{{ stat.displayValue }}</span
              ><span class="text-accent text-xl ml-1">{{ stat.suffix }}</span>
            </div>
            <div
              class="text-secondary text-[10px] sm:text-xs uppercase tracking-[0.3em] font-bold h-4 flex items-center justify-center opacity-50 group-hover:opacity-100 transition-opacity"
            >
              {{ stat.label }}
            </div>
          </div>
