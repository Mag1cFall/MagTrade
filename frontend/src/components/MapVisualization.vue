<template>
  <div
    class="relative w-full h-[400px] md:h-[600px] overflow-hidden bg-[#050505] flex items-center justify-center border border-white/5 rounded-xl group"
  >
    <!-- Grid Background & Image -->
    <img
      src="/world-map.png"
      alt="World Map"
      class="absolute inset-0 w-full h-full object-cover opacity-30 select-none pointer-events-none"
    />
    <div
    ...
    <svg viewBox="0 0 800 400" class="w-full h-full opacity-60 relative z-10">
      ...
      >
        ...
        <line
          ...
          :x1="0"
          :y1="0"
          :x2="(nodes[i + 1]?.x || 0) - node.x"
          :y2="(nodes[i + 1]?.y || 0) - node.y"
          stroke="#e33535"
          stroke-width="0.5"
          stroke-opacity="0.2"
        />
    ...
    <div class="absolute bottom-8 left-8 flex flex-col gap-2 z-20">
      ...
      <div class="text-3xl font-bold text-white tracking-tighter">
        全球 <span class="text-secondary">节点</span>
      </div>
      <div class="text-sm text-secondary font-mono">
        活跃区域: <span class="text-white">14</span> | 延迟:
        <span class="text-accent">&lt; 35ms</span>
      </div>
    </div>
  </div>
</template>
