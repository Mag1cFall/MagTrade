...
const messages = ref<{ role: 'user' | 'assistant' | 'thinking'; content: string }[]>([
  {
    role: 'assistant',
    content: '我是 MagTrade 智能战术助手。关于秒杀时机、库存状态或订单追踪，随时下达指令。',
  },
])
...
<template>
  <div class="fixed bottom-6 right-6 z-50 flex flex-col items-end">
    ...
    >
      <div
        v-if="isOpen"
        class="mb-4 w-80 md:w-96 h-[500px] bg-black/90 backdrop-blur-xl border border-white/10 flex flex-col shadow-2xl shadow-black overflow-hidden rounded-lg"
      >
        <div class="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
          ...
          <button class="text-secondary hover:text-white transition-colors" @click="toggleChat">
            <X class="w-5 h-5" />
          </button>
        </div>
        ...
        <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
          ...
          >
            ...
            >
              <div
                ...
                class="flex items-center gap-2 mb-1 text-xs text-purple-400 uppercase tracking-wider"
              >
                <Brain class="w-3 h-3" /> Thinking...
              </div>
              <div
                v-else-if="msg.role === 'assistant'"
                class="flex items-center gap-2 mb-1 text-xs text-tertiary uppercase tracking-wider"
              >
          ...
          <div
            v-if="loading && !thinkingContent && messages[messages.length - 1]?.role === 'user'"
