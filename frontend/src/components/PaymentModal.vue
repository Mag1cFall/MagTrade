  ...
  <teleport to="body">
    <transition name="modal">
      <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md bg-[#1a1a1a] border border-white/10 shadow-2xl overflow-hidden animate-in">
          <!-- 顶部渐变条 -->
          ...
          <div class="p-6">
            ...
            <div class="mb-6">
              ...
              <div class="bg-black/40 border border-white/5 rounded-lg p-4 space-y-3">
                ...
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-cyan-400 font-mono font-bold text-lg">¥{{ amount.toFixed(2) }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="mb-6">
              ...
              <div v-if="paymentMethod === 'card'" class="space-y-4">
                <div>
                  <label class="text-secondary text-xs uppercase tracking-wider block mb-1.5">Full name</label>
                  <input 
                ...
                <div>
                  <label class="text-secondary text-xs uppercase tracking-wider block mb-1.5">Card number</label>
                  <div class="relative">
                    <input 
                    ...
                    <div class="absolute right-3 top-1/2 -translate-y-1/2 flex gap-1 items-center">
                      <div class="w-8 h-5 bg-[#1A1F71] rounded flex items-center justify-center text-[6px] text-white font-bold">VISA</div>
                      <div class="w-8 h-5 bg-gradient-to-br from-[#EB001B] to-[#F79E1B] rounded flex items-center justify-center">
                        <div class="w-2 h-2 rounded-full bg-[#EB001B] -mr-1"></div>
                        <div class="w-2 h-2 rounded-full bg-[#F79E1B]"></div>
                      </div>
                      <div class="w-8 h-5 bg-white rounded flex items-center justify-center text-[5px] text-blue-600 font-bold">JCB</div>
                      <div class="w-8 h-5 bg-[#006FCF] rounded flex items-center justify-center text-[5px] text-white font-bold">AMEX</div>
                    </div>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <label class="text-secondary text-xs uppercase tracking-wider block mb-1.5">Expiration</label>
                    <input 
                  ...
                  <div>
                    <label class="text-secondary text-xs uppercase tracking-wider block mb-1.5">CVC</label>
                    <div class="relative">
                      <input 
                        ...
                        class="w-full bg-black border border-white/10 rounded-lg px-4 py-3 text-white placeholder-gray-600 focus:border-accent outline-none transition-colors font-mono"
                      />
                      <CreditCard class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-600" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div v-else-if="paymentMethod === 'alipay'" class="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 text-center">
                <Wallet class="w-8 h-8 text-blue-500 mx-auto mb-2" />
                <p class="text-blue-400 text-sm">You will be redirected to Alipay</p>
              </div>
              <div v-else-if="paymentMethod === 'wechat'" class="bg-green-500/10 border border-green-500/30 rounded-lg p-4 text-center">
                <svg class="w-8 h-8 text-green-500 mx-auto mb-2" viewBox="0 0 24 24" fill="currentColor"><path d="M8.691 2.188C3.891 2.188 0 5.476 0 9.53c0 2.212 1.17 4.203 3.002 5.55a.59.59 0 0 1 .213.665l-.39 1.472c-.078.295-.003.617.213.828a.72.72 0 0 0 .88.121l1.994-.984a.587.587 0 0 1 .504-.023c.87.351 1.82.555 2.809.596-.156-.484-.239-.996-.239-1.524 0-3.61 3.494-6.546 7.79-6.546.291 0 .578.013.86.039C16.704 5.103 13.032 2.188 8.691 2.188zm-2.51 4.66a1.073 1.073 0 1 1 0-2.145 1.073 1.073 0 0 1 0 2.146zm5.082 0a1.073 1.073 0 1 1 0-2.145 1.073 1.073 0 0 1 0 2.146z"/><path d="M23.99 16.234c0-3.252-3.262-5.891-7.283-5.891-4.02 0-7.282 2.64-7.282 5.891 0 3.252 3.262 5.891 7.282 5.891.88 0 1.721-.13 2.498-.368a.49.49 0 0 1 .421.019l1.664.82a.6.6 0 0 0 .734-.101.59.59 0 0 0 .177-.69l-.325-1.227a.49.49 0 0 1 .177-.553c1.527-1.12 2.537-2.828 2.537-4.791zm-9.633-.902a.894.894 0 1 1 0-1.789.894.894 0 0 1 0 1.79zm4.7 0a.894.894 0 1 1 0-1.789.894.894 0 0 1 0 1.79z"/></svg>
                <p class="text-green-400 text-sm">Scan QR code with WeChat</p>
              </div>
              <div v-else-if="paymentMethod === 'apple'" class="bg-white/5 border border-white/20 rounded-lg p-4 text-center">
                <svg class="w-8 h-8 text-white mx-auto mb-2" viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
                <p class="text-white/70 text-sm">Continue with Apple Pay</p>
              </div>
              <div v-else-if="paymentMethod === 'google'" class="bg-blue-400/10 border border-blue-400/30 rounded-lg p-4 text-center">
                <svg class="w-8 h-8 mx-auto mb-2" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
            ...
            <div class="flex items-start gap-3 mb-6">
              ...
              <p class="text-xs text-secondary leading-relaxed">
                ...
                <router-link to="/terms" class="text-accent hover:underline">Terms</router-link> apply.
              </p>
            </div>
...
.modal-enter-from .animate-in, .modal-leave-to .animate-in {
