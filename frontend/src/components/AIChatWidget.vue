        ...
        >
          <div class="p-4 border-b border-white/10 flex items-center justify-between bg-black/50">
            ...
            <div class="flex items-center gap-2">
              <button
                class="text-secondary hover:text-accent p-1"
                title="清空对话"
                @click="clearChat"
              >
                <Trash2 class="w-4 h-4" />
              </button>
              <button class="text-secondary hover:text-white p-1" @click="isOpen = false">
                <X class="w-5 h-5" />
              </button>
            </div>
          </div>
            ...
            >
              ...
              >
                <div
                  ...
                  class="text-xs text-secondary mb-1 italic"
                >
                  Thinking...
                </div>
                <div
...
const messages = ref<Message[]>([
  {
    role: 'assistant',
    content:
      "Hi! I'm your AI shopping assistant. Ask me about flash sales, product recommendations, or trading strategies!",
  },
])
...
const sendMessage = async () => {
  ...
  try {
    ...
    await sendChatMessageStream(sessionId.value, userMessage, authStore.token || '', (chunk) => {
    ...
    messages.value.push({
      role: 'assistant',
      content:
        "Sorry, I'm having trouble connecting. Please check your login status and try again.",
    })
  } finally {
